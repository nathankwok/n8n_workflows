...
// Sort each customer's array by month ascending; sort in place
Object.values(training_dataset).forEach(arr => {
...
// grouped is: { "<customer-id>": [rows sorted by month asc], ... }
...
Object.values(target_dataset).forEach(arr => {
    // Filter out items with invalid or missing billing_month
    const filteredItems = arr.filter(
        item => typeof item.billing_month === 'string' && item.billing_month.trim() !== ''
    );
